<template>
  <div id="app">
    <Bar
      @loginClicked="isLogin = $event"
      @registerClicked="isRegister = $event"
      :user="user"
      @logout="isLogout = $event"
    />

    <Register
      v-if="isRegister"
      @register="registerData = $event"
      @closeRegister="isRegister = false"
    />
    ...
    <Header
      ...
      @showMovieId="showMovieId = $event"
      @modal="showMovieId = $event"
    />
...
export default {
  ...
  watch: {
    showMovieId: function() {
      ...
      fetch(
        `https://api.themoviedb.org/3/movie/${this.showMovieId}?api_key=485dd1f1ee71083619712efed20ee4bb&language=en-US`
      )
        .then(resp => resp.json())
        .then(resp => {
          this.showMovie = resp;
        ...
        db.collection("watchlists")
          ...
          .then(querySnapshot => {
            if (querySnapshot.exists) {
              querySnapshot
                .data()
                .watchlist.forEach(movie => this.watchlist.push(movie));
            } else {
              db.collection("watchlists")
                .doc(this.user.uid)
                .set({
  ...
  mounted() {
    fetch(
      `https://api.themoviedb.org/3/trending/movie/day?api_key=485dd1f1ee71083619712efed20ee4bb`
    )
      .then(resp => resp.json())
      .then(resp => {
        this.randomMovie =
    ...
    fetch(
      `https://api.themoviedb.org/3/trending/movie/week?api_key=485dd1f1ee71083619712efed20ee4bb`
    )
      .then(resp => resp.json())
      .then(resp => {
...
.wl-enter-active,
.wl-leave-active {
  transition: transform 0.5s cubic-bezier(0.69, 0.5, 0, 1);
}
